...
rm(list = ls());
#
# In this exercise you will create some simulated data and will fit simple linear 
# regression models to it. Make sure to use set.seed(1) prior to starting part (a) to
...
# obtained. How do b0 and b1 compare to β0 and β1?
reg_1 <- lm(y1 ~ x1)
summary(reg_1)
...
reg_poly1 <- lm(y1 ~ x1 + I(x1^2))
summary(reg_poly1)
#
# (h) Repeat (a)–(f) after modifying the data generation process in such a way that
# there is less noise in the data. The model (3.39) should remain the same. You can do
# this by decreasing the vari- ance of the normal distribution used to generate the 
# error term ε in (b). Describe your results.
set.seed(1)
...
# there is more noise in the data. The model (3.39) should remain the same. You can do
# this by increasing the variance of the normal distribution used to generate the 
# error term ε in (b). Describe your results.
set.seed(1)
